...
func (s *userService) Create(ctx context.Context, request domain.CreateUserInput) (domain.User, error) {
	...
	eventData := domain.EventUserCreatedData{UserID: user.ID}
	if err := s.kafkaProducer.Produce(ctx, domain.UserCreatedEvent, eventData); err != nil {
		return domain.User{}, err
	}
	...
	).InfoContext(ctx, "kafka event published")
...
func (s *userService) Update(ctx context.Context, request domain.UpdateUserInput) (domain.User, error) {
	...
	eventData := domain.EventUserUpdatedData{
		UserID: user.ID,
		Rate:   user.Rate,
	}
	if err := s.rabbitmqPublisher.PublishQueue(ctx, domain.UserUpdatedEvent, eventData); err != nil {
		return domain.User{}, err
	}
	...
	).InfoContext(ctx, " rabbitmq event was published")
