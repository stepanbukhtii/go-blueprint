package service

import (
	"context"
	"log/slog"
	"strings"

	"github.com/samber/do/v2"
	"github.com/stepanbukhtii/easy-tools/econtext"
	"github.com/stepanbukhtii/easy-tools/nats"
	"github.com/stepanbukhtii/easy-tools/rest/api"
	"github.com/stephenafamo/bob/dialect/psql"
	"github.com/stephenafamo/bob/dialect/psql/sm"

	"github.com/stepanbukhtii/go-blueprint/internal/repository/postgres/models"

	"github.com/stepanbukhtii/go-blueprint/internal/clients/user"
	"github.com/stepanbukhtii/go-blueprint/internal/domain"
	"github.com/stepanbukhtii/go-blueprint/internal/repository"
	"github.com/stepanbukhtii/go-blueprint/internal/service/events"
)

type companyService struct {
	repo          repository.Repository
	userClient    user.Client
	natsPublisher *nats.Publisher
}

func NewCompanyService(injector do.Injector) (domain.CompanyService, error) {
	return &companyService{
		repo:          do.MustInvoke[repository.Repository](injector),
		userClient:    do.MustInvoke[user.Client](injector),
		natsPublisher: do.MustInvoke[*nats.Publisher](injector),
	}, nil
}

func (s *companyService) ListPaginate(ctx context.Context, request api.Pagination) ([]domain.Company, int64, error) {
	return s.repo.Company().FindAllPaginate(ctx, request)
}

func (s *companyService) Get(ctx context.Context, id string) (domain.Company, error) {
	return s.repo.Company().Find(ctx, models.SelectWhere.Companies.ID.EQ(id))
}

func (s *companyService) GetCompanyByOwner(ctx context.Context) ([]domain.Company, error) {
	clientInfo := econtext.ClientInfo(ctx)
	if clientInfo.Subject == "" {
		return []domain.Company{}, domain.ErrCompanyNotFound
	}
	return s.repo.Company().FindAll(ctx, models.SelectWhere.Companies.OwnerID.EQ(clientInfo.Subject))
}

func (s *companyService) Create(ctx context.Context, request domain.CreateCompanyInput) (domain.Company, error) {
	clientInfo := econtext.ClientInfo(ctx)

	company := domain.Company{
		Name:    request.Name,
		OwnerID: clientInfo.Subject,
		LogoURL: request.LogoURL,
	}

	if request.ManagerID != "" {
		managerUser, err := s.userClient.One(ctx, request.ManagerID)
		if err != nil {
			return domain.Company{}, err
		}
		company.ManagerID = managerUser.ID
	}

	if err := s.repo.Company().Add(ctx, &company); err != nil {
		return domain.Company{}, err
	}

	return company, nil
}

func (s *companyService) CreateMultiple(
	ctx context.Context,
	request []domain.CreateCompanyInput,
) ([]domain.Company, error) {
	var companies []domain.Company

	err := s.repo.RunInTransaction(
		ctx, func(tx repository.Repository) error {
			for _, req := range request {
				company := domain.Company{
					Name:      req.Name,
					OwnerID:   req.OwnerID,
					ManagerID: req.ManagerID,
					LogoURL:   req.LogoURL,
				}
				if err := tx.Company().Add(ctx, &company); err != nil {
					return err
				}
				companies = append(companies, company)
			}

			return nil
		},
	)
	if err != nil {
		return nil, err
	}

	return companies, nil
}

func (s *companyService) Update(ctx context.Context, request domain.UpdateCompanyInput) (domain.Company, error) {
	company, err := s.repo.Company().Find(ctx, sm.Where(models.Companies.Columns.ID.EQ(psql.Arg(request.CompanyID))))
	if err != nil {
		return domain.Company{}, err
	}

	if request.Name != "" {
		company.Name = strings.TrimSpace(request.Name)
	}

	if request.LogoURL != "" {
		company.LogoURL = request.LogoURL
	}

	if err := s.repo.Company().Update(ctx, &company); err != nil {
		return domain.Company{}, err
	}

	eventData := events.NewEventCompanyUpdatedData(company)
	if err := s.natsPublisher.Publish(ctx, events.CompanyUpdatedEvent, eventData); err != nil {
		return domain.Company{}, err
	}

	slog.With(
		slog.String("company.id", company.ID),
		slog.String("event.name", events.CompanyUpdatedEvent),
	).InfoContext(ctx, "nats event was published")

	return company, nil
}

func (s *companyService) Delete(ctx context.Context, id string) error {
	return s.repo.Company().Remove(ctx, id)
}
