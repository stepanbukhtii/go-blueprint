...
import (
	...
	"github.com/stepanbukhtii/go-blueprint/internal/transport/http/handlers/ws"
)
...
func (s *Server) registerUserTypeHandlers(r *gin.RouterGroup) {
	h := handlers.NewUserType(s.services.UserType)
...
func (s *Server) registerWSHandlers(r *gin.RouterGroup) {
	h := ws.NewNotification(s.config.WebSocket.AllowedOrigins)

	wsGroup := r.Group("/ws")
	{
		wsGroup.GET("/notifications", s.jwtMiddleware.Auth, h.Connect)
	}
}
