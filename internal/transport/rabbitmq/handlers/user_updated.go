...
func DeclareUserUpdate(declare rabbitmq.Declare) error {
	if err := declare.Queue(events.UserUpdatedEvent); err != nil {
		return err
	}
...
func (h UserUpdate) Handle(ctx context.Context, msg amqp.Delivery) error {
	var request events.EventUserUpdatedData
	...
	).InfoContext(ctx, "start update rate")

	if err := h.service.UpdateRate(ctx, request.UserID, request.Rate); err != nil {
		return err
	}
